...
@section('content')
    ...
    <div class="card mb-5 mb-x-10">
        ...
        <div class="card-header border-0 cursor-pointer" role="button" data-bs-toggle="collapse"
            ...
            <div class="card-title m-0">
                <h3 class="fw-bold m-0">{{ __('Fast shipping cities list') }}</h3>
            </div>
            <!--end::Card title-->
        </div>
        <!--begin::Card header-->
            ...
            <table id="kt_datatable" class="table align-middle text-center table-row-dashed fs-6 gy-5">
                <thead>
                    <tr class=" text-gray-400 fw-bold fs-7 text-uppercase gs-0">
                        ...
                        <th style="text-align: center">{{ __('Shipping price') }}</th>
    ...
    <form id="crud_form" class="ajax-form" action="{{ route('dashboard.fast-shipping-city.store') }}" method="post"
            ...
            <div class="modal-dialog modal-dialog-scrollable">
                <div class="modal-content">
                    <div class="modal-header">
                        <h5 class="modal-title" id="form_title">{{ __('Add new fast shipping city') }}</h5>
                        <!--begin::Close-->
                    ...
                    <div class="modal-body">
                        ...
                        <div class="fv-row mb-0 fv-plugins-icon-container">
                            <label for="city_id_inp"
                                class="form-label required fs-6 fw-bold mb-3 d-block">{{ __('City') }}</label>
                            <select class="form-select form-select-solid" data-control="select2" id="city_id_inp"
                        ...
                        <div class="fv-row mb-0 fv-plugins-icon-container">
                            <label for="shipping_price_inp"
                                class="form-label required fs-6 fw-bold mb-3 d-block">{{ __('Shipping price') }}</label>
                            <input type="number" name="shipping_price"
                                class="form-control form-control-lg form-control-solid" id="shipping_price_inp"
                                placeholder="{{ __('Shipping price') }}">
                            <div class="fv-plugins-message-container invalid-feedback" id="shipping_price"></div>
                        </div>
                    </div>
...
@push('scripts')
    ...
    <script src="{{ asset('assets/dashboard/js/datatables/fast-cities.js') }}"></script>
    <script src="{{ asset('assets/dashboard/js/global/crud-operations.js') }}"></script>

    <script>
        $(document).ready(function() {

            modalEl = document.querySelector('#city_id_inp');
            ...
            $("#add_btn").click(function(e) {
                e.preventDefault();
                $("#form_title").text(__('Add new fast shipping city'));
                $("[name='_method']").remove();
                $("#crud_form").trigger('reset');
                $("#crud_form").attr('action', `/dashboard/fast-shipping-city`);
                $("#city_id_inp").select2({
