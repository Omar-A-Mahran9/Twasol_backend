            ...
            <div class="d-flex flex-column gap-7 gap-lg-10">
                <div class="d-flex flex-wrap flex-stack gap-5 gap-lg-10">
                    ...
                    @if ($order->paying_off == 1)
                        @if (App\Enums\OrderStatus::OrderPlaced->name == $order->status)
                            <form action="{{ route('vendor.change-order-status', $order->id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="status" value="{{ App\Enums\OrderStatus::Processing->value }}">
                                <button type="submit" class="btn btn-success btn-sm me-lg-n7"
                                    style="background-color: #DEB893">{{ __('Prepare Products') }}</button>
                            </form>
                        ...
                        @elseif(App\Enums\OrderStatus::Processing->name == $order->status)
                            <form action="{{ route('vendor.change-order-status', $order->id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="status" value="{{ App\Enums\OrderStatus::Shipped->value }}">
                                <button type="submit" class="btn btn-success btn-sm me-lg-n7"
                                    style="background-color: #DEB893">{{ __('Ship Products') }}</button>
                            </form>
                            {{--  @elseif(App\Enums\OrderStatus::Shipped->name == $order->status)
                            <form action="{{ route('vendor.change-order-status', $order->id) }}" method="POST">
                                @csrf
                        ...
                        @endif
                        <!--end::Button-->
                        {{--  @dd(App\Enums\OrderStatus::PaymentConfirmed->name != $order->status || App\Enums\OrderStatus::Delivered->name != $order->status)  --}}
                        @if (App\Enums\OrderStatus::OrderPlaced->name == $order->status ||
                                App\Enums\OrderStatus::Processing->name == $order->status ||
                                App\Enums\OrderStatus::Shipped->name == $order->status)
                            <form action="{{ route('vendor.change-order-status', $order->id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="status" value="{{ App\Enums\OrderStatus::Rejected->value }}">
                                <button type="submit"
                                    class="btn btn-danger btn-sm me-lg-n7">{{ __('Cancel Order') }}</button>
                            </form>
                        @endif
                    @else
                        @if (App\Enums\OrderStatus::OrderPlaced->name == $order->status)
                            <form action="{{ route('vendor.change-order-status', $order->id) }}" method="POST">
                                ...
                                <input type="hidden" name="status"
                                    value="{{ App\Enums\OrderStatus::Processing->value }}">
                                <button type="submit" class="btn btn-success btn-sm me-lg-n7"
                                    style="background-color: #DEB893">{{ __('Prepare Products') }}</button>
                            </form>
                        @elseif(App\Enums\OrderStatus::Processing->name == $order->status)
                    ...
                    <a href="{{ route('vendor.products.index') }}"
                        class="btn btn-primary btn-sm">{{ __('Go to products page') }}</a>
                    <!--end::Button-->  --}}
                </div>
                <!--begin::Order summary-->
                                        ...
                                        <table class="table align-middle table-row-dashed fs-6 gy-5 mb-0">
                                            ...
                                            <tbody class="fw-semibold text-gray-600">
                                                            ...
                                                            <div class="d-flex align-items-center">
                                                                ...
                                                                <div
                                                                    ...
                                                                    <div
                                                                        ...
                                                                        {{ strtoupper(substr($orderItem->product->name, 0, 2)) }}
                                                                    </div>
                                                                </div>
                                                                <!--end::Thumbnail-->
                                                                <!--begin::Title-->
                                                                <div class="ms-5">
                                                                    <a href="{{ route('vendor.products.show', $orderItem->product->id) }}"
                                                ...
                                                {{--  <tr>
                                                    ...
                                                    <td class="text-end">${{ $shippingPrice }}</td>
                                                </tr>
                                                <tr>
                                                    <td colspan="3" class="fs-3 text-dark text-end">
                                                        {{ __('Grand Total') }}</td>
                                                    <td class="text-dark fs-3 fw-bolder text-end">
                                                        {{ $subTotal + $shippingPrice }}</td>
                                                </tr>  --}}
                                            </tbody>
                                        </table>
                                        <!--end::Table-->
